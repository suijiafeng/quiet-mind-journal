...
function AppRoutes() {
  ...
  const nextSuggestion = checkInItems.find((item) => item.status === '未打卡')
    ? {
        title: `优先完成 ${checkInItems.find((item) => item.status === '未打卡')?.name}`,
        detail: checkInItems.find((item) => item.status === '未打卡')?.id === 'work'
          ? '先把今天最重要的任务拆清楚，再进行完成打卡。'
          : checkInItems.find((item) => item.status === '未打卡')?.id === 'sleep'
            ? '补充昨晚睡眠记录，分析页的数据才会准确。'
            : '先完成最靠前的未打卡项目，首页进度会同步推进。',
        path: checkInItems.find((item) => item.status === '未打卡')?.path ?? '/checkins',
      }
    : undefined;
  ...
  return (
    <Layout>
      <Routes>
        <Route path="/" element={<Home progress={{ completed: homeCompleted, total: homeCards.length || 1 }} suggestion={nextSuggestion} cards={homeCards} />} />
        <Route path="/checkin/morning" element={<MorningCheckIn checkedIn={state.morning.checkedIn} checkedInAt={state.morning.checkedInAt} history={state.morning.history} onCheckIn={() => setState((current) => ({ ...current, morning: { ...current.morning, checkedIn: true, checkedInAt: nowTime(), history: uniqueSortedDates([...current.morning.history, todayKey()]) } }))} />} />
        <Route path="/checkin/sleep" element={<SleepCheckIn sleepAt={state.sleep.sleepAt} wakeAt={state.sleep.wakeAt} durationMinutes={state.sleep.durationMinutes} last7Days={state.sleep.last7Days} onSleep={() => setState((current) => ({ ...current, sleep: { ...current.sleep, sleepAt: nowTime() } }))} onWake={() => setState((current) => { const wakeAt = nowTime(); const durationMinutes = getSleepDurationMinutes(current.sleep.sleepAt, wakeAt) ?? current.sleep.durationMinutes ?? 450; const entry = { date: todayKey().slice(5), durationMinutes, sleepTime: current.sleep.sleepAt ?? '--:--', wakeTime: wakeAt }; return { ...current, sleep: { ...current.sleep, wakeAt, durationMinutes, last7Days: [...current.sleep.last7Days.filter((item) => item.date !== entry.date), entry].slice(-7) } }; })} />} />
        <Route path="/checkin/work" element={<WorkPlan tasks={state.work.tasks} checkedIn={state.work.checkedIn} completionRate={state.work.completionRate} onToggleTask={(id) => setState((current) => { const tasks = current.work.tasks.map((task) => (task.id === id ? { ...task, completed: !task.completed } : task)); const doneCount = tasks.filter((task) => task.completed).length; return { ...current, work: { ...current.work, tasks, completionRate: calculateCompletionRate(doneCount, tasks.length) } }; })} onAddTask={(text) => setState((current) => { const tasks = [...current.work.tasks, { id: `${Date.now()}`, text: text.trim(), completed: false }]; const doneCount = tasks.filter((task) => task.completed).length; return { ...current, work: { ...current.work, tasks, completionRate: calculateCompletionRate(doneCount, tasks.length) } }; })} onCheckIn={() => setState((current) => { const doneCount = current.work.tasks.filter((task) => task.completed).length; return { ...current, work: { ...current.work, checkedIn: true, checkedInAt: nowTime(), completionRate: calculateCompletionRate(doneCount, current.work.tasks.length) } }; })} />} />
        ...
        <Route path="/checkin/custom/:id" element={<CustomCheckInPage items={state.customCheckIns} onCheckIn={(id, note) => setState((current) => ({ ...current, customCheckIns: current.customCheckIns.map((item) => { if (item.id !== id) return item; const hasToday = item.history.includes(todayKey()); return { ...item, completedToday: true, checkedInAt: nowTime(), note, streak: hasToday ? item.streak : item.streak + 1, history: uniqueSortedDates([...item.history, todayKey()]) }; }) }))} />} />
        ...
        <Route path="/settings" element={<SettingsPage state={state} onUpdate={(updater) => setState((current) => ({ ...current, ...updater }))} onUpdateSettings={(key, value) => setState((current) => ({ ...current, settings: { ...current.settings, [key]: value } }))} onBackup={() => downloadFile(`quiet-mind-journal-backup-${todayKey()}.json`, JSON.stringify(state, null, 2), 'application/json')} onImport={importBackup} onReset={resetAppState} />} />
      </Routes>
    </Layout>
  );
}
